<script lang="ts">
  ...
  // Component Props
  export let grid: number = 15;
  export let mouse: number = 0.1;
  export let strength: number = 0.15;
  export let relaxation: number = 0.9;
  export let imageSrc: string;
  
  // Handling the reserved 'class' keyword in Svelte
...
`;
  ...
  onMount(() => {
    ...
    const animate = () => {
      ...
      const textureData: Float32Array = dataTexture.image.data;
      for (let i = 0; i < size * size; i++) {
    ...
    // Svelte runs this returned function when the component is unmounted
    return () => {
      if (animationId) {
